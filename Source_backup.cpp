...
bool cmp_starttime(Node* b,  Node* a)
{
...
int main()
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                Node* tmp_pointer = NULL;

                cmd_input.erase(cmd_input.begin()); //pop off first command
                //<task>[0] <priority>[0] <length>[2] <arrives>[3] 
                tmp_pointer = new Node(cmd_input[0], std::stoi(cmd_input[1]), std::stoi(cmd_input[2]), std::stoi(cmd_input[3]));
            ...
            else if (cmd_input[0] == "step" && simulate)
            {
                ...
                if (cmd_input.size() > 0)
                {
                    ...
                    for (sim_time; sim_time < sim_stop_time; sim_time++)
                    {
                        ...
                        if (active_task == NULL && (!heap.empty() && sim_time == 0))
                        {
                        ...
                        if (active_task != NULL && active_task->start_time <= sim_time)
                        {
                            active_task->elapsed_time += 1;
                        }
                        // check to see if task is complete

                        if (active_task != NULL && active_task->elapsed_time == active_task->length)
                        ...
                        if (active_task == NULL && !heap.empty())
                        {
                            //moves task with highest priority to back
                            std::pop_heap(heap.begin(), heap.end(), cmp_starttime);
                            // sets active_task to highest priority task
